...
\begin{LIST}{1cm}
  ...
  {
    询问用户，根据回答返回 \retval{\T} 或
    \retval{\NIL}。关于 \VAR{control} 和 \VAR{arg}s，参见第
    \pageref{section:Format} 页 \FU{format}。
  }

  \IT{(\MC*{WITH-STANDARD-IO-SYNTAX} \PROGN{\VAR{form}})}
  {
    以标准的读取打印行为求值 \VAR{form}s。返回
  ...
  {
    继续读取，直至遇到 \VAR{char}。返回已读取对象的
    \retval{列表}。若 stream 中未发现 \VAR{char}，则产生错误。
  }
  ...
  \IT{(\FU*{READ-CHAR-NO-HANG}
    \OP{\DES{\VAR{stream}}\DF{\V{\A standard-input\A}}
      \OP{\VAR{eof-error}\DF{\T} \Op{\VAR{eof-val}\DF{\NIL}
          \Op{\VAR{recursive}\DF{\NIL}}}}})}
  {
    \VAR{stream} 的 \retval{下一字节}，若无，返回
    \retval{\NIL}。
  }
  ...
  \IT{(\FU*{UNREAD-CHAR} \VAR{character}
    \Op{\DES{\VAR{stream}}\DF{\V{\A standard-input\A}}})}
  {
    将最后 \FU{read-char} 的 \VAR{character} 放回 \VAR{stream}，返回
    \retval{\NIL}。
  }
  ...
  \IT{(\FU*{READ-BYTE} \DES{\VAR{stream}} \OP{\VAR{eof-err}\DF{\T}
      \Op{\VAR{eof-val}\DF{\NIL}}})}
  {
    从二进制 \VAR{stream} 中读取 \retval{下一字节}。
  }
  ...
  {
    返回 \retval{\VAR{from-readtable} 副本}。
  }
...
\begin{LIST}{1cm}
  ...
  {
    \VAR{foo}s 的列表，终止 cdr 为 \VAR{bar}。
  }
  ...
  {
    $m.n\cdot10^x$，\kwd*{short-float}、\kwd*{single-float}、\kwd*{double-float}、\kwd*{long-float}
    或 \kwd{\A
      read-default-float-format\A} 定义的类型。
  }
...
\begin{LIST}{1cm}
  ...
  {
    将 \retval{\VAR{object}} 打印至 \VAR{stream}。由 Lisp
    打印器调用。
  }
...
\begin{LIST}{1cm}
  ...
  \begin{LIST}{.5cm}
    ...
    \IT{\Goo{\KWD{\TLD C}\XOR\KWD{\TLD :C}\XOR\KWD{\TLD
          @C}\XOR\KWD{\TLD @:C}}}
    {\index{\~C@$\sim$C}%
      \EM{字符。}将参数作为（可能为不可打印）字符并打印、指出、以
      \kwd{\#$\backslash$}
      语法打印、指出键入方法。
    }
